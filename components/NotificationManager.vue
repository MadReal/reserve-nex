...
const sentence = computed(() => activeNotification.value?.text)
const defaultErrorMsg = ref("")

if (!activeNotification.value?.isSuccess && sentence.value === "") {
  defaultErrorMsg.value = isIT
...
<template>
  <Transition>
    ...
    >
      <p class="text-sm">{{ sentence || defaultErrorMsg }}</p>
      <div
