...
export const useDiscountsStore = defineStore("DiscountsStore", () => {
  ...
  async function deleteDiscountAmount(discountAmountId: DiscountAmount["id"]) {
    const { status } = await useFetch(`${URL_discountAmount}/${discountAmountId}`, {
      method: "delete",
      headers: { Authorization: authToken.value ? `Bearer ${authToken.value}` : "" },
    })
  ...
  async function deleteDiscount(discountId: Discount["id"]) {
    const { status } = await useFetch(`${URL_discount}/${discountId}`, {
      method: "delete",
      headers: { Authorization: authToken.value ? `Bearer ${authToken.value}` : "" },
    })
    if (status.value === "error") return storeNotifications.openNotification("", false)
  ...
  async function deleteAllDiscountsOnDayOfWeek(dayOfWeek: Discount["dayOfWeek"]) {
    const { status } = await useFetch(
      `${URL_discountDayOfWeek}?dayOfWeek=${dayOfWeek}&restaurantId=${activeRestaurantId.value}`,
      { method: "delete", headers: { Authorization: authToken.value ? `Bearer ${authToken.value}` : "" } },
    )
    if (status.value === "error") return storeNotifications.openNotification("", false)
