...
export const useRestaurantsStore = defineStore(
  ...
  () => {
    ...
    const restaurantsIsLiveList = computed(() =>
      restaurantsList.value.filter((item) => item.isLive),
    );
    ...
    async function fetchRestaurants() {
      ...
      if (fetchedRestaurants) {
        ...
        const isActiveRestaurantIdInsideFetched = fetchedRestaurants.some(
          (item) => item.id === activeRestaurantId.value,
        );
        // set new activce restuarant id to null => user goes to /restuarant page to set new active one
        if (
          !isActiveRestaurantIdInsideFetched &&
          fetchedRestaurants.length >= 1
        )
    ...
    async function fetchSingleRestaurant(restaurantId: Restaurant["id"]) {
      const { data, error } = await useFetch<Restaurant>(
        `${URL}/${restaurantId}`,
      );
      if (data && data.value) {
